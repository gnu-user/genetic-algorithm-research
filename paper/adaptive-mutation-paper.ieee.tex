...
% FIGURE - 8 QUEENS DISTINCT SOLUTION
\begin{figure}[h!]
...
\begin{algorithm}
  ...
  \tcp{Use arbitrary sorting algorithm}
...
\section{Experiments}
We measured the performance of our self-adaptive approach using an empirical study. Our methodology was to solve N-Queens for various values of N with a limited budget of generations. Number of function calls (\emph{NFC}) is given as $P \times Generations$ where $P = 64$ as specified in Table \ref{table:geneticoperators}, as the fitness evaluation function is called $P$ times per generation. Each experiment was repeated a multitude of times to ensure accuracy and collect aggregate statistics. Table \ref{table:trials} shows the number of generations permitted and the number of trials run for each value of $N$. Special budgets and trial multiplicities were set for $N = 32$ in order to accommodate the exceptionally large problem size.
...
{\renewcommand{\arraystretch}{1.2}t c
\begin{tabular}{|l|l|l|l|} \hline
&       \multicolumn{2}{c|}{Number of Function Calls (\emph{NFC})}& \\ \cline{2-3}
$N$&    Deterministic \cite{cit:21}&  Self-Adaptive\footnote{\emph{NFC} is given as $P \times Generations$, where $P = 64$.}&                       \% Difference \\ \hline
...
\begin{tabular}{|l|l|l|} \hline
$M_{c}$&         Number of Generations&      \% Difference\footnote{\emph{\% Difference} is given as Generations $\frac{M_{c} - \text{Self-Adaptive}}{\text{Self-Adaptive}} \times 100\%$}\\ \hline
...
\bibitem{cit:24} R.G. Spallek, T.B. Preu{\ss}er and B. N{\"a}gel, ``QUEENS@TUD: The World Record by FPGAs!,"
