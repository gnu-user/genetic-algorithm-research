...
Classic Genetic Algorithms (GA) have been used extensively as a means to optimize some task Genetic and evolutionary algorithms come of age][Genetic Algorithms: a survey][Using Genetic algorithms to solve np-complete problems]. A popular problem to test GA on is the N-Queens problem [The N-Queens Problem and Genetic Algorithms][Solving the n-Queens Problem using Genetic Algorithms]. However as stated in [AUTHOR Solving the n-Queens Problem using Genetic Algorithms], the N-Queens problem is a very different problem from most NP-problems. Since the fitness is binary, there is no particular direction is search in since there is no feedback besides correct or incorrect. 

One major concern with using GA is determining the best genetic operators, the probability mutation and crossover. Values for these operators are defined prior to the execution and remain static in the classical GA. [AUTHOR Towards an Optimal Mutation Probability for Genetic Algorithms] attempt is made to algorithmically defined suitable genetic operators [Towards an Optimal Mutation Probability for Genetic Algorithms]

Attempts to improve the selection of these genetic operations have been made over the last few decades. Since Genetic Algorithms are inspired by real life the adjustments tend to be based in adding more nature based augmentations to improve the performance of GA. One major example of this is Adaptive Genetic Algorithms (AGA) which generally, is an attempt to adjust the genetic operations actively to escape local optimum faster and reach maximums quicker. An typical example of AGA uses a defined measurement of the current 'performance' of the population by evaluating the population's fitness. Use of these AGAs requires less knowledge of the search space, the better the fitness the lower the mutation and alternatively the worse the fitness the higher the mutation [Genetic Algorithms and Directed Adaptation] [Adaptive probabilities of crossover and mutation in genetic algorithms]. 

More specifically, [AUTHOR Genetic Algorithms and Directed Adaptation] changed the only the mutation rate between two values, a 'hypermutable' (high rate), and normal (typical mutation rate) [Genetic Algorithms and Directed Adaptation]. Use of the fitness function for the population provided the indicator for switching between the two values. [AUTHOR Adaptive probabilities of crossover and mutation in genetic algorithms] provides a similar example however, the genetic operator are not restricted to two values, rather they are calculated and thus allow for more adjustment [Adaptive probabilities of crossover and mutation in genetic algorithms]. A bit of a different genetic algorithm is described in [AUTHOR Adapting Operator Settings in Genetic Algorithms] which accounts for each member in the population to have its own defined genetic operators. The genetic operators will be adapted in the creation of the new generation [Adapting Operator Settings in Genetic Algorithms]

[AUTHORS Some improvements on adaptive genetic algorithms for reliability-related applications] gives a fundamental look at adaptive genetic algorithms by comparing two major, contradicting processes: mutation-first, and crossover-first. Mutation-first relies on high mutation rate initially with a low crossover rate, over time however it will progress to the opposite, low mutation rate and high crossover rate. Crossover-first is the exact opposite, low mutation and high crossover initially and high mutation with low crossover for the end. It was found that the mutation-first method out performed the crossover-first [Some improvements on adaptive genetic algorithms for reliability-related applications]. This can be attributed to the need to explore the search space early in the execution (through mutation) and to make slight adjustments (through crossover) later on [Some improvements on adaptive genetic algorithms for reliability-related applications]. An attempt is also made to provide a plausible method for solution to the local optimum problem by introducing randomly generated individuals to the population to increase diversity [Some improvements on adaptive genetic algorithms for reliability-related applications].
